...
def getArticles():
    cursor = connectToDb()
    sql = "SELECT id, title, description FROM contacts order by title"
    cursor.execute(sql)
...
def showArticle(id):
    cursor = connectToDb()
    sql = f"SELECT content FROM contacts where id = '{id}'"
    cursor.execute(sql)
...
def postList():    
    cursor = connectToDb()
    sql = "SELECT * FROM contacts order by title"
    cursor.execute(sql)
    results = cursor.fetchall()
    return results
...
def updatePost(data):
    ...
    for k, v in data.items():
        ...
        sets += k + ' = "' + v + '", '
    sets = sets.rstrip(', ')
    qry = 'UPDATE contacts '  + sets + ' where id = ' + str(data.get('id'))
    try:
        cursor.execute(qry)
        db.commit()
        rct = cursor.rowcount
...
def deletePost(data):
    cur = connectToDb()
    idee = data.get('id')
    qry = 'delete from contacts where id =' + str(idee)
    try:
        cur.execute(qry)
        db.commit()
        rct = cur.rowcount
